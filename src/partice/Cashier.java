...
public class Cashier {
    public int n;
    public int discount;
    public Map<Integer,Integer> pToP = new HashMap<>();
    public int index;
    ...
    public double getBill(int[] product, int[] amount) {
        ...
        if(index%n == 0){
            for (int i = 0;i < product.length;i++){
                sum += ( pToP.get(product[i]) -  pToP.get(product[i]) * ((double)discount/100) ) * amount[i];
            }
        }else {
            for (int i = 0;i < product.length;i++){
                sum += ((double)  pToP.get(product[i]) * amount[i]);
            }
        }
        index++;
        return sum;
