...
/**
 * @author chengj
 * @Description 1898. 可移除字符的最大数目 中等
 * @Date 2022/6/20
 */
...
public class MaximumRemovals {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(mr.maximumRemovals("abcab","abc",new int[]{0,1,2,3,4}));
    }
    public int maximumRemovals(String s, String p, int[] removable) {
        int l = -1;
        //K 可以取到 removable.length
        int r = removable.length + 1;
        while(l + 1 != r){
            int mid = l + (r - l)/2;
            if(checkSub(s,p,removable,mid)){
                l = mid;
            }else{
                r = mid;
            }
    ...
    private boolean checkSub(String s, String p, int[] removable, int mid) {
        char[] chars = s.toCharArray();
        for (int i=0; i< mid; ++i) {
            //to upper
            chars[removable[i]] -= 32;
        }
        char[] charsP = p.toCharArray();
        int j = 0;
        for (int i=0; i<s.length(); ++i) {
            if (Character.isLowerCase(chars[i]) && chars[i] == charsP[j]) {
                ++j;
                if (j == p.length()) {
                    return true;
                }
